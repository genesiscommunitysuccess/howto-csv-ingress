...
views {
  view("EXCHANGE_HOLIDAY_VIEW", EXCHANGE_HOLIDAY) {
    joins {
      joining(EXCHANGE, backwardsJoin = true) {
        on(EXCHANGE_HOLIDAY { ISO_MIC } to EXCHANGE { ISO_MIC })
      }
    }
    fields {
      EXCHANGE_HOLIDAY.ISO_MIC
      EXCHANGE_HOLIDAY.HOLIDAY_DATE
      EXCHANGE_HOLIDAY.HOLIDAY_TYPE
      EXCHANGE_HOLIDAY.HOLIDAY_NAME
      EXCHANGE.NAME
      EXCHANGE.ISO_COUNTRY
    }
  }
}
